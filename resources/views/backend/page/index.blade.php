...
<a href="{{url(BACKEND_PATH.'page.create')}}" class="btn btn-primary d-none d-sm-inline-block" data-toggle="" data-title="Administrator Group | Add New" data-class="modal-lg">
	Add New
</a>
@endsection
...
<div class="row">
    <div class="col">
        <form role="form" action="{{url(BACKEND_PATH.'page.delete')}}" method="post" id="ajxFormDelete">
            <div class="card">
                <div class="card-body">
...
<script>
    $(document).ready(function() {
        var table = $('#dataNav').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ url(BACKEND_PATH.'page.data') }}",
            },
            columns: [{
                    data: 'status',
                    name: 'status',
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        var id = row.id;
                        var editUrl = "{{url(BACKEND_PATH.'page.edit')}}";
                        return `<a href="${editUrl}?id=${id}"  data-title="Page | Edit" data-class="modal-lg">Edit</a>`;
        ...
        function sendOrderToServer() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "{{url(BACKEND_PATH.'page.post-sortable')}}",
                data: {
                    order: order,
                    _token: token
                },
